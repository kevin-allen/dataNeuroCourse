...
6. git: create your git repository on github
...
```{r sevGraphs2, fig.width = 5,fig.height=2}
...
```{r sevGraphs22, fig.width = 5,fig.height=2}
df1 %>% 
  ggplot() +
...
```{r sevGraphs10, fig.width = 5,fig.height=2}
df1 %>% 
...
We have created several interesting graphs. But how do you 
...
```{r sevGraphs4, fig.width = 3,fig.height=2}
df1 <- df %>% group_by(mouse,block) %>% 
  summarise(performance = mean(correct))
df1 %>% 
  ggplot() +

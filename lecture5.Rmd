...
Complex models can learn to perform great on your training set but generalize very poorly to new data. This is called **overfitting**.
...
We need to adjust our loss function to return one value when we pass several pairs of number as input.
...
```{r training}
params <- torch_tensor(c(1.0,0.0), requires_grad = TRUE)
trainingLoop(n_epochs = 10, learning_rate = 0.0001,model=model, params=params, tu = tu,tc=tc)
params
```
...
trainingLoop <- function(n_epochs=10,learning_rate=0.0001
  ...
  for (epoch in seq(1,n_epochs)){
   ...
   if(epoch < 3 | epoch > n_epochs - 2) # print info 
...
There are several excellent machine learning course online (datacamp)
